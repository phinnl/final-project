...
pub trait ProvideEnergy<F: Fuel> {
    ...
    fn provide_energy_with_efficiency(&self, f: FuelContainer<F>, e: u8) -> <F as Fuel>::Output {
        F::Output::from(f.amount * F::energy_density().into() * (e as u32) / 100)
    }
...
pub struct InternalCombustion<const DECAY: u32>(RefCell<u8>, RefCell<u32>);
...
impl<const DECAY: u32, F: Fuel> ProvideEnergy<F> for InternalCombustion<DECAY> {
    fn provide_energy(&self, f: FuelContainer<F>) -> <F as Fuel>::Output {
        ...
        let mut time_decrease = self.1.borrow_mut();
        if *time_decrease == 0 {
            *time_decrease = DECAY;
            *efficiency = if *efficiency > 0 { *efficiency - 1 } else { 0 };
        };
        *time_decrease -= 1;
...
impl<const C: u8, F1: Fuel, F2: Fuel> Fuel for CustomMixed<C, F1, F2> {
    ...
    fn energy_density() -> Self::Output {
        (F1::energy_density().into() * (C as u32) / 100)
...
pub fn omni_80_energy(amount: u32) -> BTU {
    let omni_80 = OmniGenerator::<80>;
    omni_80.provide_energy(FuelContainer::<Mixed<Diesel, LithiumBattery>>::new(amount))
}
...
mod tests {
    ...
    fn green_should_work() {
        let ge = GreenEngine::<Diesel>(PhantomData);
        assert_eq!(
            ge.provide_energy(FuelContainer::<Diesel>::new(10)).to_btu(),
            1000
        );
    }
    ...
    fn british_should_work() {
        let bt = BritishEngine::<Uranium>(PhantomData);
        assert_eq!(
            bt.provide_energy(FuelContainer::<Uranium>::new(10))
                .to_btu(),
            10000
        );
    }
